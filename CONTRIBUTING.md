...
1. use the search functionality [here](https://github.com/Pjieter/drippy/issues) to see if someone already filed the same issue;
...
1. [push](http://rogerdudler.github.io/git-guide/) your feature branch to (your fork of) the drippy repository on GitHub;
...
1. Bump the version using `bump-my-version bump <major|minor|patch>`. For example, `bump-my-version bump major` will increase major version numbers everywhere it's needed (code, meta, etc.) in the repo. Alternatively the version can be manually changed in drippy/__init__.py, pyproject.toml, CITATION.cffand docs/conf.py (and other places it was possibly added).
...
1. Wait until [PyPi publish workflow](https://github.com/Pjieter/drippy/actions/workflows/publish.yml) has completed
...
1. Wait until new release is also on Bioconda (https://anaconda.org/bioconda/drippy) via a automaticly created PR on [bioconda recipes repo](https://github.com/bioconda/bioconda-recipes/pulls?q=is%3Apr+is%3Aopen+drippy)
1. Test drippy from bioconda by manually running [Conda verify](https://github.com/Pjieter/drippy/actions/workflows/conda_verify.yml) workflow
-->
